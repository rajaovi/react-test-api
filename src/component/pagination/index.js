...
const Pagination = ({totalCount, pageSize, startEndData, activePageNum=1}) => {
    ...
    return (
        <div className="pagination">
            <button onClick={e => handleFirstPage(e)} className={`firstLast firstPage ${disablePageFirstBtn  ? 'disableBtn' : ''}`}></button>
            ...
            <button onClick={e => handleLastPage(e)} className={`firstLast lastPage ${disablePageLastBtn  ? 'disableBtn' : ''}`}></button>
        </div>
    )
}
